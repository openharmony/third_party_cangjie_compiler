...
struct Node {
     ...
     * Problems: 1 only js backends uses this field? 2 It should be the ScopeManager's duty that set this field?
     */
...
struct QuoteExpr : Expr {
    ...
    Position leftParenPos;  /**< Position of '('. */
    Position rightParenPos; /**< Position of ')'. */
    std::string content;    /**< For fmt tools. */
    /**
    ...
    TypeKind GetNumLitTypeKind()
    {
        ...
        if (kind == LitConstKind::INTEGER) {
            ...
            if (suffix.empty()) {
                ...
                try {
                    suffixWidth = std::stoi(std::string(suffix.begin() + 1, suffix.end()));
                } catch (...) {
...
struct File : Node {
    ...
    bool hasJavaFFI{false};             /**< Whether has definition modified with @Java */
